...
# Render ke liye Dummy Web Server (Taaki bot band na ho)
app = Flask('')
...
def run_web_server():
    # Render khud apna PORT decide karta hai, isliye os.environ use kiya hai
    port = int(os.environ.get("PORT", 8080))
    app.run(host='0.0.0.0', port=port)

# Token ab seedhe Render ke dashboard se secure tarike se aayega
...
def add_deal(message):
    ...
    try:
        ...
        buyer_match = re.search(r'buyer\s*[:\-=\s]*\s*(@\w+)', decoded_text)
        seller_match = re.search(r'seller\s*[:\-=\s]*\s*(@\w+)', decoded_text)
...
if __name__ == "__main__":
    # Flask Server ko alag thread mein start kar rahe hain
